...
def get_images():
    ...
    count = 199
    for link in range(199, len(all_links)):
        if count == 268:
            break
        ...
        try:
            ...
            #                         break
            #                     else:
            #                         print('Не успеваю скачать фото после ввода капчи... Жду ещё 40 секунд!')
            #                         time.sleep(40)
            #                         images = [file for file in os.listdir(directory1) if
            #                                   os.path.isfile(f'{directory1}/{file}')]
            #                         if len(images) != len(preview):
            #                             print('Через 40 секунд после ввода капчи ничего не скачало! Жду еще 30')
            #                             time.sleep(30)
            #                             print('Через 40 секунд после ввода капчи ничего не скачало! Жду еще 30')
            #                             images = [file for file in os.listdir(directory1) if
            #                                       os.path.isfile(f'{directory1}/{file}')]
            #                             if len(images) != len(preview):
            #                                 print(
            #                                     'Даже после  минуты ожидания не успеваю скачать фото после ввода капчи! Пробую заново')
            #                                 flag = True
            #                                 break
            #                         flag = False
            #                         break
            ...
            #                       os.path.isfile(f'{directory1}/{file}')]
            #             if len(images) != len(preview):
            #                 print('Не успеваю скачать фото без капчи... Жду ещё!')
            #                 flag = False
            #                 time.sleep(60)
            #                 images = [file for file in os.listdir(directory1) if
            #                           os.path.isfile(f'{directory1}/{file}')]
            #                 if len(images) != len(preview):
            #                     print('Не получилось скачать фото без капчи после 60 сек ожидания, пробую ещё раз!')
            #                     flag = True
